...
export default function Home() {
  ...
  //   { name: "David", amount: 40 },
  // ]);
  // const transactions = [
  //   { name: "Paid to Eve", amount: 25 },
  //   { name: "Received from Frank", amount: 35 },
  // ];
  type DebtDueType = { name: string; amount: number };
  ...
  const [debtType, setDebtType] = useState("due"); // Default to Due
    ...
    fetch("http://localhost:5000/UserData", {
      ...
      .then((data) => {
        ...
        const formattedDues: DebtDueType[] = Object.entries(data.due).map(
          ([name, amount]) => ({
            name,
            amount: Number(amount), // Explicitly cast amount to a number
          })
        );
        ...
        setTransactions(data.transaction || []); // Set transactions from the response
      })
  ...
  useEffect(() => {
    fetchData();
  }, [refreshKey]); // Depend on refreshKey to re-fetch data

  // Call this function whenever you want to refresh the data
  const refreshData = () => {
  ...
  // Fetch dummy data (replace this with actual API call)
  // useEffect(() => {
  //   // Simulating backend fetch
  //   setTimeout(() => {
  //     setPeople(["John Doe", "Jane Smith", "Alice Johnson"]);
  //   }, 500);
  // }, []);
  return (
    <div
      ...
      {isModalOpen && (
        <ViewModal
          ...
          editAmount={(index, newAmount) => {
            ...
            setModalData(updatedData); // Update state
          }}
          deleteEntry={(index) => {
